...
volatile uint32_t ulDummy;
	...
	AT91C_BASE_PMC->PMC_PCER = portTIMER_CLK_ENABLE_BIT;

	/* Stop the tick timer... */
	portTIMER_REG_BASE_PTR->TC_CCR = AT91C_TC_CLKDIS;
	...
	#if configUSE_PREEMPTION == 1
	{
		...
		AT91C_BASE_AIC->AIC_SVR[portTIMER_AIC_CHANNEL] = 
            ( uint32_t ) vPreemptiveTick;
	}
	#else  // else use cooperative scheduler
	{
		extern void ( vNonPreemptiveTick )( void );
		AT91C_BASE_AIC->AIC_SVR[portTIMER_AIC_CHANNEL] = 
            ( uint32_t ) vNonPreemptiveTick;
	}
	#endif

	/* Tick timer interrupt level-sensitive, priority 6... */
	AT91C_BASE_AIC->AIC_SMR[ portTIMER_AIC_CHANNEL ] = 
        AT91C_AIC_SRCTYPE_INT_LEVEL_SENSITIVE | portTICK_PRIORITY_6;

	/* Enable the tick timer interrupt...

	First at timer level */
	portTIMER_REG_BASE_PTR->TC_IER = AT91C_TC_CPCS;
	...
	if( (configCPU_CLOCK_HZ / (configTICK_RATE_HZ * 2) ) <= 0xFFFF )
	{
		...
		clock (main clock / 2). */
		portTIMER_REG_BASE_PTR->TC_CMR = 
            AT91C_TC_WAVE | AT91C_TC_CLKS_MCK_2 | 
            AT91C_TC_BURST_NONE | AT91C_TC_CPCTRG;
		portTIMER_REG_BASE_PTR->TC_RC  = 
            configCPU_CLOCK_HZ / (configTICK_RATE_HZ * 2);
	}
	else
	{
		...
		portTIMER_REG_BASE_PTR->TC_CMR = 
            AT91C_TC_WAVE | AT91C_TC_CLKS_MCK_8 | 
            AT91C_TC_BURST_NONE | AT91C_TC_CPCTRG;
		portTIMER_REG_BASE_PTR->TC_RC  = 
            configCPU_CLOCK_HZ / (configTICK_RATE_HZ * 8);
	}

	/* Start tick timer... */
	portTIMER_REG_BASE_PTR->TC_CCR = AT91C_TC_SWTRG | AT91C_TC_CLKEN;
}
/*-----------------------------------------------------------*/
